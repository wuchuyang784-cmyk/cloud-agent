...
function UserRow({ row, showAgents }: { row: Row; showAgents: (id: string) => void }) {
  return <tr><td><strong>{row.displayName || row.email}</strong><span className="subtext">{row.email}</span></td>
    ...
    <td className="date">{date(row.createdAt)}</td><td><button className="row-action" title="查看该用户的 Agent" aria-label={'查看 ' + row.email + ' 的 Agent'} onClick={() => showAgents(row.id)}><Bot size={17} /><ChevronRight size={14} /></button></td></tr>;
}
...
export function App() {
  ...
  useEffect(() => {
    const refresh = () => { if (document.visibilityState === 'visible' && model.getSnapshot().phase === 'ready') void model.load(query); };
  ...
  return <div className="admin-layout">
    ...
    <main className="content" aria-busy={busy}>
      <div className="page-heading"><div><div className="breadcrumb">平台 / {view === 'users' ? '用户账号' : 'Agent 服务'}</div><h1>{view === 'users' ? '用户账号' : 'Agent 服务'}</h1></div><span className="read-label">只读</span></div>
      ...
      <div className="table-scroll"><table><thead>{view === 'users' ? <tr><th>账号</th><th>用户 ID</th><th>认证身份</th><th>创建时间</th><th className="action-column">Agent</th></tr> : <tr><th>Agent</th><th>所属用户</th><th>记录状态</th><th>引擎</th><th>记录更新时间</th></tr>}</thead>
        <tbody>{!busy && state.items.map(row => view === 'users' ? <UserRow key={row.id} row={row} showAgents={id => { setDraft(''); reset({ view: 'agents', ownerUserId: id }); }} /> : <AgentRow key={row.id} row={row} />)}
          {(busy || state.items.length === 0) && <tr><td colSpan={5}><div className="empty" role="status">{busy ? <RefreshCw className="spin" size={24} /> : view === 'users' ? <Users size={28} /> : <Bot size={28} />}<span>{state.phase === 'signing-out' ? '正在退出' : busy ? '正在读取' : '暂无记录'}</span></div></td></tr>}
        </tbody></table></div>
