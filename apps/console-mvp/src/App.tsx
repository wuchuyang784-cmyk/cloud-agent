import { Activity, Archive, BarChart3, Bell, BookOpen, Bot, CheckCircle2, CircleAlert, Clock3, FileCheck2, LayoutDashboard, LibraryBig, LoaderCircle, MessageSquare, Plus, Puzzle, RefreshCw, Search, Send, Settings, Settings2, ShieldCheck, Sparkles, Star, Terminal, Trash2, Users, Wallet, Wrench, X, Zap } from 'lucide-react';
import type { FormEvent, ReactNode } from 'react';
import { useCallback, useEffect, useMemo, useState } from 'react';
import { addFavorite, createAgent, createFiling, createResource, createSession, deleteResource, ensureDevSession, fetchAccount, fetchAgents, fetchFavorites, fetchFilings, fetchHealth, fetchNotifications, fetchResource, fetchResources, fetchSettings, fetchTransactions, fetchUsage, markNotificationRead, readAllNotifications, rechargeAccount, removeFavorite, streamChat, updateResource, updateSettings, type AccountInfo, type BackendAgent, type BackendNotification, type BackendResource, type BillingTransaction, type FavoriteItem, type FilingRecord, type ResourceDetail, type ResourceKind, type Session, type UsagePayload, type User, type UserSettings } from './api';
...
export function App() {
  ...
  const refresh = useCallback(async () => {
    setRefreshing(true);
    try {
      const [nextAgents, nextResources, nextUsage] = await Promise.all([fetchAgents(), fetchResources(), fetchUsage('today')]);
      ...
      setError(null);
    } catch (caught) { setError(caught instanceof Error ? caught.message : '加载控制台数据失败'); }
    finally { setRefreshing(false); }
  }, []);
  useEffect(() => {
    let cancelled = false;
    void (async () => { try { const current = await ensureDevSession(); if (!cancelled) { setUser(current); await refresh(); } } catch (caught) { if (!cancelled) setError(caught instanceof Error ? caught.message : '无法连接平台 API'); } finally { if (!cancelled) setLoading(false); } })();
    void fetchHealth().then(({ database }) => { if (!cancelled) setHealth(database === 'postgres' ? 'postgres' : 'memory'); }).catch(() => undefined);
    return () => { cancelled = true; };
  }, [refresh]);
  useEffect(() => { if (!agents.some((item) => item.status === 'provisioning')) return undefined; const timer = window.setInterval(() => void refresh(), 2000); return () => window.clearInterval(timer); }, [agents, refresh]);
  async function submitAgent(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const name = agentName.trim(); if (!name) return;
    try { const created = await createAgent(name); setAgentName(''); setModal(false); setSelectedId(created.id); setPanel('agents'); await refresh(); } catch (caught) { setError(caught instanceof Error ? caught.message : '创建 Agent 失败'); }
  }
  ...
  async function submitMessage(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!text.trim() || !agent || !session || sending) return;
...
function TemplateGrid({ onCreateFromTemplate }: { onCreateFromTemplate: (name: string) => void }) { return <section className="template-grid agent-template-grid">{templateItems.map(({ name, description, tags, tone, icon: Icon }) => <button key={name} className="template-card" onClick={() => onCreateFromTemplate(name)}><span className={'template-icon ' + tone}><Icon /></span><strong>{name}</strong><p>{description}</p><span className="template-meta">{tags.map((tag) => <i key={tag}>{tag}</i>)}</span><span className="template-use">使用模板 <Chevron /></span></button>)}</section>; }
function Agents({ agents, selectedId, entryTab, favorites, onSelect, onCreate, onChat, onRefresh, onCreateFromTemplate, onToggleFavorite }: { agents: BackendAgent[]; selectedId: string | null; entryTab: AgentTab; favorites: FavoriteItem[]; onSelect: (id: string) => void; onCreate: () => void; onChat: (agent: BackendAgent) => void; onRefresh: () => void; onCreateFromTemplate: (name: string) => void; onToggleFavorite: (agent: BackendAgent) => void }) {
  ...
  return <div className="page-stack agent-workspace">
    <section className="page-heading"><div><span className="eyebrow">智能体工作区</span><h1>智能体 Agents</h1><p>从模板快速开始，或管理当前用户自己的 Agent 项目。</p></div><div className="heading-actions"><button className="button secondary" onClick={onRefresh}><RefreshCw />刷新</button><button className="button primary" onClick={onCreate}><Plus />创建 Agent</button></div></section>
...
function AgentCard({ agent, onChat }: { agent: BackendAgent; onChat: (agent: BackendAgent) => void }) { const state = statusCopy[agent.status as keyof typeof statusCopy] ?? statusCopy.provisioning; return <article className="agent-card"><div className="agent-card-top"><span className="agent-avatar"><Bot /></span><span className={'status-pill ' + state.tone}>{state.tone === 'ready' ? <CheckCircle2 /> : <Clock3 />}{state.label}</span></div><h3>{agent.name}</h3><p className="agent-id">{agent.id}</p><dl><div><dt>运行地址</dt><dd>{agent.host || 'agent-' + agent.id + '.localhost'}</dd></div><div><dt>Runtime</dt><dd>{agent.runtimeKind || 'mock'}</dd></div></dl>{agent.status === 'ready' ? <button className="button primary full" onClick={() => onChat(agent)}><MessageSquare />开始对话</button> : <div className="agent-pending"><LoaderCircle className="spin" />Worker 正在准备 Agent</div>}</article>; }
function AgentRow({ agent, selected, favorite, onSelect, onChat, onToggleFavorite }: { agent: BackendAgent; selected: boolean; favorite: boolean; onSelect: () => void; onChat: () => void; onToggleFavorite: () => void }) { const state = statusCopy[agent.status as keyof typeof statusCopy] ?? statusCopy.provisioning; return <button className={'agent-row ' + (selected ? 'selected' : '')} onClick={onSelect}><span className="agent-avatar small"><Bot /></span><span className="agent-row-main"><strong>{agent.name}</strong><small>{agent.id}</small></span><span className={'status-pill ' + state.tone}>{state.tone === 'ready' ? <CheckCircle2 /> : <Clock3 />}{state.label}</span><span className="agent-row-host">{agent.host || 'agent-' + agent.id + '.localhost'}</span><span className={'agent-star' + (favorite ? ' on' : '')} role="button" tabIndex={0} title={favorite ? '取消收藏' : '收藏'} aria-label={favorite ? '取消收藏' : '收藏'} onClick={(event) => { event.stopPropagation(); onToggleFavorite(); }} onKeyDown={(event) => { if (event.key === 'Enter') { event.stopPropagation(); onToggleFavorite(); } }}><Star /></span>{agent.status === 'ready' ? <span className="row-action" onClick={(event) => { event.stopPropagation(); onChat(); }}>对话 <Chevron /></span> : <span className="row-action muted">等待中</span>}</button>; }
function Chat({ agent, session, messages, text, sending, onText, onSubmit, onBack }: { agent: BackendAgent; session: Session; messages: ChatMessage[]; text: string; sending: boolean; onText: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onBack: () => void }) { return <div className="chat-page"><header className="chat-header"><button className="text-button" onClick={onBack}>返回 Agents</button><div className="chat-agent-title"><span className="agent-avatar small"><Bot /></span><div><h1>{agent.name}</h1><p><span className="connection-dot" />{session.title} · 模拟 Runtime</p></div></div><span className="status-pill ready"><CheckCircle2 />运行中</span></header><section className="chat-stream">{messages.map((item) => <article key={item.id} className={'chat-message ' + item.role}><span className="message-avatar">{item.role === 'user' ? '你' : item.role === 'assistant' ? <Bot /> : <Terminal />}</span><div><div className="message-meta"><strong>{item.role === 'user' ? '我' : item.role === 'assistant' ? agent.name : '系统'}</strong><time>{item.time}</time></div><p>{item.content}</p></div></article>)}</section><form className="chat-composer" onSubmit={onSubmit}><textarea value={text} onChange={(event) => onText(event.target.value)} placeholder="输入消息，开始与 Agent 对话..." rows={3} disabled={sending} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} /><button className="send-button" type="submit" title="发送消息" aria-label="发送消息" disabled={!text.trim() || sending}>{sending ? <LoaderCircle className="spin" /> : <Send />}</button></form></div>; }
...
function Empty({ onCreate }: { onCreate: () => void }) { return <div className="empty-state"><Bot /><h3>还没有 Agent</h3><p>创建你的第一个 Agent，开始验证客户端业务流程。</p><button className="button primary" onClick={onCreate}><Plus />创建 Agent</button></div>; }
function FavoritesPanel({ favorites, onOpen, onRemove, onGoCreateAgent, onGoLibrary }: { favorites: FavoriteItem[]; onOpen: (item: FavoriteItem) => void; onRemove: (item: FavoriteItem) => void; onGoCreateAgent: () => void; onGoLibrary: () => void }) { return <div className="page-stack"><section className="page-heading"><div><span className="eyebrow">快捷入口</span><h1>我的收藏</h1><p>收藏常用的 Agent 与客户端资源，随时从顶部回到业务现场。</p></div><div className="heading-actions"><button className="button secondary" onClick={onGoLibrary}><LibraryBig />浏览资源库</button><button className="button primary" onClick={onGoCreateAgent}><Plus />创建 Agent</button></div></section>{favorites.length === 0 ? <section className="empty-state"><Star /><h3>还没有收藏</h3><p>把常用的 Agent 或客户端资源加入收藏后，会在这里统一展示。</p></section> : <div className="agent-list resource-list">{favorites.map((item) => { const isAgent = item.targetType === 'agent'; const ready = item.status === 'ready' || item.status === 'active'; const label = isAgent ? (item.status === 'ready' ? '运行中' : '待就绪') : item.status === 'active' ? '使用中' : '已收藏'; return <div key={item.id} className="resource-row"><span className={'resource-icon ' + (isAgent ? 'agent' : 'kb')}>{isAgent ? <Bot /> : <LibraryBig />}</span><span className="resource-main"><strong>{item.name || (isAgent ? item.targetId : '客户端资源')}</strong><small>{isAgent ? '智能体 Agent · ' + item.targetId : '资源库对象 · ' + item.targetId}</small></span><span className={'resource-kind ' + (isAgent ? 'agent' : 'kb')}>{isAgent ? 'Agent' : '资源'}</span><span className={'status-pill ' + (ready ? 'ready' : 'pending')}>{ready ? <CheckCircle2 /> : <Clock3 />}{label}</span><span className="resource-actions"><button type="button" className="text-button" onClick={() => onOpen(item)}>打开 <Chevron /></button><button type="button" className="text-button danger" onClick={() => onRemove(item)}><Star />取消收藏</button></span></div>; })}</div>}</div>; }
function BillingPanel({ account, transactions, onTopUp }: { account: AccountInfo | null; transactions: BillingTransaction[]; onTopUp: (amountCents: number) => Promise<void> }) {
  const [amount, setAmount] = useState('100');
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const yuan = (cents: number) => (cents / 100).toFixed(2);
  async function topUp() {
    const cents = Math.round(Number(amount) * 100); if (!(cents > 0) || busy) return;
  ...
  return <div className="page-stack"><section className="page-heading"><div><span className="eyebrow">平台服务</span><h1>费用中心</h1><p>账户余额用于支付 Agent 对话消耗。当前 MVP 以模拟 Runtime 固定单价计费，对话结束后自动扣费。</p></div></section><section className="metric-grid"><Metric icon={Wallet} label="账户余额" value={'¥ ' + yuan(account?.balanceCents ?? 0)} detail={account ? '可用余额（元）' : '账户数据暂不可用'} /><Metric icon={Zap} label="对话单价" value="¥ 0.0001" detail="每次对话固定消耗" /><Metric icon={Activity} label="累计流水" value={String(transactions.length)} detail="充值 + 消费记录" /></section><section className="panel-card"><div className="section-heading"><div><h2>账户充值</h2><span>按元输入金额，将写入当前用户账户</span></div></div><div className="recharge-row">{['50', '100', '500'].map((value) => <button key={value} type="button" className={'chip' + (amount === value ? ' active' : '')} onClick={() => { setAmount(value); setMessage(null); }}>¥{value}</button>)}<input className="recharge-input" value={amount} inputMode="decimal" aria-label="充值金额（元）" onChange={(event) => { setAmount(event.target.value); setMessage(null); }} /><span className="recharge-unit">元</span><button type="button" className="button primary" disabled={busy || !(Number(amount) > 0)} onClick={() => void topUp()}>{busy ? <LoaderCircle className="spin" /> : <Wallet />}立即充值</button></div>{message && <p className="inline-message">{message}</p>}</section><section className="panel-card"><div className="section-heading"><div><h2>收支明细</h2><span>最近的充值与消费记录</span></div></div>{transactions.length === 0 ? <div className="empty-state small"><Wallet /><h3>暂无交易记录</h3><p>充值或与 Agent 对话后，这里会展示账户变动。</p></div> : <div className="tx-list">{transactions.map((item) => <div key={item.id} className="tx-row"><span className={'tx-icon ' + item.type}>{item.type === 'recharge' ? <Wallet /> : <Zap />}</span><span className="tx-main"><strong>{item.type === 'recharge' ? '账户充值' : item.description || 'Agent 对话消耗'}</strong><small>{item.id} · {formatTime(item.createdAt)}</small></span><span className={'tx-amount ' + item.type}>{item.type === 'recharge' ? '+' : '-'}¥{yuan(item.amountCents)}</span><span className="tx-after">余额 ¥{yuan(item.balanceAfterCents)}</span></div>)}</div>}</section></div>;
}
