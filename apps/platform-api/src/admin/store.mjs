...
export async function readAdmin(store, actor, resource, query = {}) {
  ...
  if (resource === 'me') return { role: binding.role, permissions, user: safeUser(store.users.get(actor)) };
  const rows = resource === 'users' ? [...store.users.values()].map(safeUser) : [...store.agents.values()].map(agent => ({
