...
const infrastructureCheck = process.argv.includes('--infrastructure');
const governanceCheck = process.argv.includes('--governance');
const platformCheck = process.argv.includes('--platform') || adminCheck || clientMonitoringCheck || infrastructureCheck || governanceCheck;
...
try {
  ...
  const code = await new Promise((resolve, reject) => {
    const tests = governanceCheck ? ['apps/platform-api/test/governance.test.mjs', 'apps/platform-api/test/governance-postgres.test.mjs'] : infrastructureCheck ? ['scripts/infrastructure-collector.test.mjs', 'apps/platform-api/test/infrastructure.test.mjs', 'apps/platform-api/test/infrastructure-postgres.test.mjs'] : clientMonitoringCheck ? ['apps/platform-api/test/client-monitoring.test.mjs', 'apps/platform-api/test/client-monitoring-postgres.test.mjs'] : adminCheck ? ['apps/platform-api/test/admin.test.mjs', 'apps/platform-api/test/admin-postgres.test.mjs'] : platformCheck
